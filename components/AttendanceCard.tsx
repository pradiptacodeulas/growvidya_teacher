...
export default function AttendanceCard({ data }: AttendanceCardProps) {
  ...
  const attendanceStats = [
    { value: data?.present || 0, color: '#4CAF50', label: 'Present' },
    { value: data?.late || 0, color: '#2196F3', label: 'Late' },
    { value: data?.half || 0, color: '#FF9800', label: 'Half Day' },
    { value: data?.absent || 0, color: '#F44336', label: 'Absent' },
  ];

  const totalDays = (data?.present || 0) + (data?.late || 0) + (data?.half || 0) + (data?.absent || 0);
    ...
    <View style={[styles.container, { backgroundColor: colors.cardBg, borderColor: colors.border }]}>
      ...
      <View style={[styles.weekBox, { backgroundColor: colors.surfaceSubtle, borderColor: colors.border }]}>
        ...
        <View style={styles.daysRow}>
          {weekDays.map((day, index) => (
            <View key={index} style={[styles.dayBadge, { backgroundColor: colors.cardBg, borderColor: colors.border }]}>
      ...
      <View style={[styles.statsTable, { borderColor: colors.border }]}>
        <View style={styles.statColumn}>
          ...
          <Text style={[styles.statValue, { color: '#4CAF50' }]}>{data?.present || 0}</Text>
        </View>
        <View style={[styles.statDivider, { backgroundColor: colors.border }]} />
        <View style={styles.statColumn}>
          <Text style={[styles.statLabel, { color: colors.textMuted }]}>Late</Text>
          <Text style={[styles.statValue, { color: '#2196F3' }]}>{data?.late || 0}</Text>
        </View>
        <View style={[styles.statDivider, { backgroundColor: colors.border }]} />
        <View style={styles.statColumn}>
          <Text style={[styles.statLabel, { color: colors.textMuted }]}>Half Day</Text>
          <Text style={[styles.statValue, { color: '#FF9800' }]}>{data?.half || 0}</Text>
        </View>
        <View style={[styles.statDivider, { backgroundColor: colors.border }]} />
        <View style={styles.statColumn}>
          <Text style={[styles.statLabel, { color: colors.textMuted }]}>Absent</Text>
          <Text style={[styles.statValue, { color: '#F44336' }]}>{data?.absent || 0}</Text>
        </View>
      </View>
