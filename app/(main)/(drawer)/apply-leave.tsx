...
export default function ApplyLeaveScreen() {
  ...
  const leaveTypesData = leaveTypes.map((type) => ({
    id: type.id,
    name: `${type.leave_name} (Quota: ${type.no_leave})`,
  }));
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
        ...
        <View style={[styles.formCard, { backgroundColor: colors.cardBg, borderColor: colors.border }]}>
          ...
          <View style={styles.inputGroup}>
            ...
            <TouchableOpacity 
              ...
              onPress={() => leaveTypeBottomSheetRef.current?.present()}
              activeOpacity={0.8}
            >
              <Text style={[styles.dropdownValueText, { color: colors.text }, !selectedLeaveType && { color: colors.textMuted }]}>
                {selectedLeaveType 
                  ? `${selectedLeaveType.leave_name} (Quota: ${selectedLeaveType.no_leave})` 
                  : 'Select Leave Type'}
              </Text>
      ...
      <SelectionBottomSheet
        ...
        onSelect={(item) => {
          const type = leaveTypes.find(t => t.id === String(item.id));
          if (type) {
            setSelectedLeaveType(type);
            // Clear selected document if new type doesn't need it
            if (String(type.need_document) !== '1') {
              setSelectedDocument(null);
            }
          }
        }}
