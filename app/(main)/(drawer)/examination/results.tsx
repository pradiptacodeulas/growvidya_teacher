...
export default function ExamResultsScreen() {
  ...
  const fetchYears = useCallback(async () => {
    ...
    try {
      ...
      if (Array.isArray(list) && list.length > 0) {
        const normalized: AcademicYearItem[] = list.map((y: any) => ({
          id: String(y.id),
          academic_year: y.academic_year || y.name || y.title || String(y.id),
          is_current: String(y.is_current ?? y.isCurrent ?? '0'),
          status: String(y.status ?? '1'),
        }));
        ...
        const currentYear =
          normalized.find(
            (y: AcademicYearItem) =>
              ...
              String(y.is_current).toLowerCase() === "true"
          ) || normalized[0];
        if (currentYear) {
          setSelectedYear((prev) => prev || String(currentYear.id));
        }
      } else {
  ...
  const yearData = useMemo(() => {
    return academicYears.map((y) => {
      const yearLabel = String(y.academic_year || "").trim();
      const displayName = yearLabel.toLowerCase().includes("academic")
        ? yearLabel
        : yearLabel
  ...
  const renderResultItem = ({ item }: { item: StudentResultItem }) => {
    ...
    return (
      <View style={[styles.resultCard, { backgroundColor: colors.cardBg, borderColor: colors.border }]}>
        ...
        {isExpanded && (
          <View style={[styles.expandedContent, { backgroundColor: colors.surfaceSubtle }]}>
            <View style={[styles.divider, { backgroundColor: colors.border }]} />
            <Text style={[styles.detailsTitle, { color: colors.textSecondary }]}>Subject Breakdown</Text>
            ...
            >
              <Text style={[styles.tableHeaderCell, { flex: 2, color: colors.textSecondary }]}>Subject</Text>
              <Text style={[styles.tableHeaderCell, { flex: 1.2, textAlign: 'center', color: colors.textSecondary }]}>Marks</Text>
              <Text style={[styles.tableHeaderCell, { flex: 1, textAlign: 'center', color: colors.textSecondary }]}>Grade</Text>
            </View>
