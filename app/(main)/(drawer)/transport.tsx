...
export default function TransportScreen() {
  ...
  const renderHeader = () => {
    ...
    const activeCount = transportData.filter(
      (item) => Number(item.status ?? item.staus) === 1
    ).length;
          ...
          <View style={[styles.statBox, { backgroundColor: colors.cardBg, borderColor: colors.border, marginLeft: 8 }]}>
            ...
            <View style={{ flex: 1 }}>
              ...
              <Text style={[styles.statValue, { color: colors.text }]}>
                {activeCount > 0 ? 'Active' : transportData.length > 0 ? (Number(transportData[0].status ?? transportData[0].staus) === 4 ? 'Soft Delete' : 'Inactive') : 'Unassigned'}
              </Text>
            </View>
          </View>
        </View>
  ...
  const getStatusInfo = (statusVal?: string | number) => {
    const s = Number(statusVal);
    if (s === 1) {
      return {
        label: 'Active',
        dotStyle: styles.activeDot,
        textStyle: isDark ? { color: '#4ade80' } : styles.activeText,
        badgeStyle: isDark ? { backgroundColor: 'rgba(22, 163, 74, 0.2)' } : styles.activeBadge,
      };
    }
    if (s === 2) {
      return {
        ...
        textStyle: isDark ? { color: '#f87171' } : styles.inactiveText,
        badgeStyle: isDark ? { backgroundColor: 'rgba(220, 38, 38, 0.2)' } : styles.inactiveBadge,
      };
    }
    if (s === 4) {
      return {
        ...
        textStyle: isDark ? { color: '#cbd5e1' } : { color: '#64748b' },
        badgeStyle: isDark ? { backgroundColor: 'rgba(148, 163, 184, 0.2)' } : { backgroundColor: '#f1f5f9', borderColor: '#cbd5e1' },
      };
    }
    return {
      label: 'Inactive',
  ...
  const renderTransportItem = ({ item }: { item: TransportAllocation }) => {
    const statusInfo = getStatusInfo(item.status ?? item.staus);
